...
const blobToDataUrl = (blob: Blob): Promise<string> =>
  new Promise((resolve, reject) => {
    ...
    const fail = () => reject(new Error('Could not read the export resource'));
...
export const parseUnicodeRanges = (value: string): IExportCodePointRange[] =>
  splitCssList(value).flatMap((range) => {
    const [, start, end] = UNICODE_RANGE_PATTERN.exec(range) ?? [];
    if (!start) return [];

    return [
      {
        start: Number.parseInt(start.replaceAll('?', '0'), 16),
        end: Number.parseInt(end ?? start.replaceAll('?', 'F'), 16),
      },
    ];
  });
...
const toFontFace = (rule: CSSFontFaceRule, sheet: CSSStyleSheet): IExportFontFace => ({
  ...
  sheets.flatMap((sheet) => {
    try {
      return Array.from(sheet.cssRules).flatMap((rule) => {
        ...
        if (rule instanceof CSSFontFaceRule) return [toFontFace(rule, sheet)];
...
const inlineFontResources = async (
  ...
  const replacements = await Promise.all(
    Array.from(css.matchAll(CSS_URL_PATTERN), async ([match, raw = '']) => {
      const source = raw.trim().replace(QUOTES_PATTERN, '');
